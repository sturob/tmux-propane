...
tmux bind -N " * y-maximize pane"  -n  M-m  run-shell -b "$CURRENT_DIR/maximize.sh"
tmux bind -N " * y-minimize pane"  -n  M-n  run-shell -b "$CURRENT_DIR/trim.sh"
tmux bind -N " * y-minimize pane"  -n  M-N  run-shell -b "$CURRENT_DIR/trim.sh -d"
tmux bind -n  M-"=" select-layout -E
